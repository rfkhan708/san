...
namespace Client
{
    ...
    public partial class frmNetBook : Form
    {
     ...
     var TempList = csv.CSV_Class.cimlist.Where(a => a.Token == tok).ToList();
       ...
       //Math.Round(  Global.Instance.NetBookTable.AsEnumerable().Sum(x => x.Field<double>("LTP")),4)
      //  Global.Instance.NetBookTable.AsEnumerable().Sum(x => x.Field<int>("StrikePrice"))
        }
